...
BNO_Status_t BNO055_GetCalibrationOffsets(BNO055Init_TypeDef_t *dev, uint8_t *buffer)
{
	HAL_StatusTypeDef status;
	    ...
	    /* * BNO055'in kalibrasyon verilerini okuyabilmek için sensörün
	     * CONFIG_MODE'da olması daha sağlıklıdır. Ancak NDOF modunda da
	     * okuma yapılabilir. Eğer hata alırsan önce CONFIG'e çekmelisin.
	     */

	    // 0x55 adresinden başla, 22 byte oku
	    // I2C adresi struct içinden geliyor, Timeout'u da oradan alıyoruz.
	    status = HAL_I2C_Mem_Read(dev->i2cHandler,
	                             (dev->i2cAddress),
	                             0x55,
...
BNO_Status_t BNO055_SetCalibrationOffsets(BNO055Init_TypeDef_t *dev, const uint8_t *buffer)
{
    ...
    /* DİKKAT: Kalibrasyon verisi yazmak için sensör MUTLAKA CONFIG_MODE'da olmalıdır! */
    // Burada mod değiştirme fonksiyonunu çağırdığını varsayıyorum:
    BNO055_SetOperationMode(dev, BNO_MODE_CONFIG);
    dev->delayCallback(25); // Mod geçişi için kısa bir bekleme

    status = HAL_I2C_Mem_Write(dev->i2cHandler,
                              (dev->i2cAddress ),
                              0x55,
                              I2C_MEMADD_SIZE_8BIT,
                              (uint8_t *)buffer,
    ...
    // İşlem bitince eski moda geri dön (Örn: NDOF)
    BNO055_SetOperationMode(dev, dev->operationMode);
    dev->delayCallback(25);

    return (status == HAL_OK) ? BNO_OK : BNO_I2C_ERROR;
}
